  ...
  // Открыть единое окно настроек (вкладки: активность / пароль / входы / уведомления / чёрный список / скрытые авторы)
  window.openSettingsModal = function (tab) {
    profileDropdown.classList.remove('show');
    settingsModal.classList.add('show');
    switchSettingsTab(tab || 'activity');
  };
  ...
  window.switchSettingsTab = function (tab) {
    const tabs = { activity: 'stTabActivity', password: 'stTabPassword', logins: 'stTabLogins', notifs: 'stTabNotifs', blacklist: 'stTabBlacklist', hidden: 'stTabHidden' };
    const panes = { activity: 'stPaneActivity', password: 'stPanePassword', logins: 'stPaneLogins', notifs: 'stPaneNotifs', blacklist: 'stPaneBlacklist', hidden: 'stPaneHidden' };
  ...
  function registerPostView(msgId) {
    ...
    window.api.viewMessage(msgId).then(() => {
      const cachedMsg = currentMessagesList.find(m => m.id === msgId);
  ...
  // (Открытие истории активности теперь происходит через switchSettingsTab('activity') внутри openSettingsModal)
  ...
  function openUserProfile(userId, fallbackName, fallbackAvatar) {
    ...
    if (isMe) {
      ...
      if (providerIconEl) {
        ...
        providerIconEl.classList.remove('via-telegram', 'via-email');
        providerIconEl.classList.add(viaTelegram ? 'via-telegram' : 'via-email');
        ...
        providerIconEl.innerHTML = viaTelegram
          ...
          : `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4"><rect x="2" y="4" width="20" height="16" rx="3"/><path d="m2.5 6.5 9.5 7 9.5-7"/></svg>`;
      }
    } else {
    ...
    msgsContainer.innerHTML = sorted.map(m => {
      ...
      return `
        <div class="np-profile-post-card">
          <div class="np-profile-post-time">${timeInfo.display} ${linkBadge}</div>
  ...
  window.toggleSubscription = function(targetId, targetName, opts) {
    ...
    window.api.toggleSubscribe(targetId).then((res) => {
      const nowSubscribed = res.subscribed;
      showToast(nowSubscribed ? `Вы подписались на @${targetName}` : `Вы отписались от @${targetName}`);
      applySubscribeBadge(opts.badgeEl, nowSubscribed);
    }).catch(err => console.error(err));
  };

  function applySubscribeBadge(badgeEl, nowSubscribed) {
    if (badgeEl) {
      badgeEl.classList.toggle('subscribed', nowSubscribed);
      badgeEl.title = nowSubscribed ? 'Отписаться' : 'Подписаться';
      badgeEl.innerHTML = nowSubscribed
        ? `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>`
        : `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg>`;
    }
  ...
  // ===== ПЕРЕНЕСЕНО НА CLOUDFLARE =====
  // Firebase убран. Вместо firebase.auth()/firestore() используем window.api (см. api.js),
  // который ходит на твой Cloudflare Worker. Сессия хранится в httpOnly-cookie на сервере,
  // поэтому глобальной переменной auth/db с "текущим пользователем" больше не нужно —
  // просто держим currentUser в памяти и обновляем через window.api.me().
  let currentUser = null; // { id, username, email, avatarUrl, ... } или null для гостя
  ...
  // ===== НОВАЯ ВЕРСИЯ (Cloudflare) =====
  // Вызывается автоматически виджетом Telegram после успешного входа.
  // Вся проверка подписи и мульти-Telegram-аккаунты больше не нужны —
  // Worker сам проверяет hash и хранит один аккаунт на telegram_id.
  window.onTelegramAuth = function (telegramUser) {
    window.api.loginTelegram(telegramUser)
      .then(async () => {
        await refreshCurrentUser();
        closeLoginModal();
        closeRegisterModal();
        showToast(`Вы вошли как ${currentUser.username}`);
        sortAndRender();
        renderVideosGrid();
      })
      .catch((err) => {
        if (err.error === 'banned') {
          showBanOverlay(err.banReason, err.bannedAt);
        } else {
  ...
  // ===== СТАРАЯ ВЕРСИЯ (Firebase, оставлена ниже закомментированной ссылкой для истории) =====
  window.onTelegramAuth_OLD_FIREBASE = function (telegramUser) {
  ...
  */
  ...
  window.selfPinMessage = function(id) {
    if (!currentUser || !canSelfPin()) {
      showToast('Закрепление уже использовано в этом периоде', 'error');
      return;
    }
    const pinnedUntil = Date.now() + 60 * 60 * 1000; // 1 час
    window.api.admin.pinMessage(id, pinnedUntil).then(() => {
      localStorage.setItem('lastSelfPinAt_' + currentUser.id, String(Date.now()));
  ...
  function registerUser() {
    ...
    // ===== Cloudflare: регистрация через Worker вместо Firebase =====
  ...
  function loginUser() {
    ...
    // ===== Cloudflare: вход через Worker вместо Firebase =====
    window.api.login(email, password)
      ...
      .catch((error) => {
        if (error.error === 'banned') {
          showBanOverlay(error.banReason, error.bannedAt);
        } else {
  ...
  // Вход через Twitch временно убран по запросу (был на Firebase OIDC-провайдере).
  // Кнопку в index.html тоже нужно удалить/скрыть — см. MIGRATION_STATUS.md.
  window.loginWithTwitch = function() {
    showToast('Вход через Twitch временно недоступен', 'error');
  };
  ...
  window.toggleFriend = function(targetUserId) {
    ...
    window.api.toggleFriend(targetUserId).then((res) => {
      showToast(res.friend ? 'Пользователь добавлен в друзья!' : 'Пользователь удален из друзей');
