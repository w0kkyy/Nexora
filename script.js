  ...
  */
  ...
  window.closeSettingsModal = function () {
    const o = document.getElementById('stOverlay');
    if (o) o.remove();
  };
  ...
  window.openSettingsModal = function () {
    ...
    overlay.className = 'ni-overlay';
    overlay.innerHTML = `
      <div class="st-box" role="dialog" aria-modal="true" aria-label="Настройки">
        <div class="st-side">
          <div class="st-side-title">Настройки</div>
          <button type="button" class="st-side-item active">Скрытые авторы</button>
        </div>
        <div class="st-main">
          <button type="button" class="ni-close st-close" aria-label="Закрыть">✕</button>
          <div id="stPane"></div>
        </div>
      </div>`;
    document.body.appendChild(overlay);
    overlay.addEventListener('click', e => { if (e.target === overlay) closeSettingsModal(); });
    overlay.querySelector('.st-close').onclick = closeSettingsModal;
    renderHiddenAuthorsPane();
  };
